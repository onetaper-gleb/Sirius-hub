...
class Schedule:
    ...
    def __init__(self):
        ...
        self.__lessons = [0, 60 * 10 + 5, 60 * 11 + 40, 60 * 13 + 15, 60 * 14 + 50,
    ...
    async def classroom_list(self, meaning, next=False):
        if type(meaning) != str or type(next) != type(False):
            raise TypeError('Meaning must be a string. Type of variable next must be Bull.')
        ...
        try:
            async with session.get('https://schedule.siriusuniversity.ru/classroom') as resp:
        ...
        try:
            async with session.post('https://schedule.siriusuniversity.ru/livewire/message/classroom.classroom-main-grid', headers=headers, json=payload) as response:
            ...
            for value in classroomlist.values():
                if ('Сириус' in value) or ('К_У' in value) or ('Корпус "Спорт"' in value):
                    continue
                elif 'Альфа' in value:
                    ...
                    # if value.count('К_') == 2:
                    #     second_index = value.find('К_', first_index + len('К_'))
                    if value.count('.') < 2:
                        new_value = value[first_index:]
                    elif value.count('.') == 2:
    ...
    async def get_classrooms(self):
        self.__all_classrooms = {}
        await asyncio.gather(self.classroom_list("альфа"), self.classroom_list("К"), self.classroom_list("бета"), self.classroom_list("дельта"))
    ...
    async def update_classroom_list(self):
        need_update = ((not self.__classrooms) or (self.__updated_classrooms is None) or (datetime.now() - self.__updated_classrooms > timedelta(days=7)))
        if need_update:
    ...
    async def get_free_classrooms(self, date=datetime.now()):
        ...
        all_classrooms = await asyncio.gather(*(self.classroom(classroom) for classroom in self.__classrooms[:20]))
        all_classrooms_2 = await asyncio.gather(*(self.classroom(classroom) for classroom in self.__classrooms[20:40]))
        all_classrooms_3 = await asyncio.gather(*(self.classroom(classroom) for classroom in self.__classrooms[40:]))
        ...
        free_classrooms = [self.__all_classrooms[key] for key in keys_for_free_classrooms]
